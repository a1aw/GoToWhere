...
self.addEventListener("message", function (evt) {
    ...
    db["stop_times_raw"].where("[package+provider]").equals([pkg, provider]).first().then(function (row) {
        ...
        while (start < end) {
            ...
            val = chunk[mid];
            com = stopId.localeCompare(val[headerIndex]);
        ...
        if (chunk[start][headerIndex] === stopId) {
            for (i = start; i < chunk.length; i++) {
                if (chunk[i][headerIndex] !== stopId) {
                    break;
                }
